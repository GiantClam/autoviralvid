...
OUT_DIR = Path("../test_outputs/qa_round7").resolve()
...
async def main() -> None:
    ...
    (OUT_DIR / "pipeline_result_round7.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    render_payload = (((payload.get("artifacts") or {}).get("render_payload")) or {})
    (OUT_DIR / "render_payload_round7.json").write_text(
    ...
    if pptx_b64:
        (OUT_DIR / "deck_round7.pptx").write_bytes(base64.b64decode(pptx_b64))
