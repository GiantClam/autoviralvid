...
import PPTPromptWorkspace from '@/components/PPTPromptWorkspace';
...
function ProjectWorkspace({
  ...
  const isPptWorkspace = initialTemplateId === 'ppt-generator' && !activeRunId;

  if (isPptWorkspace) {
    return (
      <div className="flex flex-1 h-full overflow-hidden">
        <PPTPromptWorkspace embedded />
      </div>
    );
  }
...
function HomeContent() {
  ...
  const isPptWorkspace = !resolvedActiveRunId && selectedTemplateId === 'ppt-generator';
    ...
    <ProjectProvider>
      <div className="flex flex-col md:flex-row h-screen w-full bg-[#050508] overflow-hidden font-sans text-gray-100">
        ...
        <div className="md:hidden shrink-0 border-b border-white/[0.06] bg-black/40 backdrop-blur-xl">
          ...
          {!isPptWorkspace ? <ProgressPanel compact /> : null}
        </div>
        ...
        {!isPptWorkspace ? <AIAssistant /> : null}
      </div>
    </ProjectProvider>
  );
}
